import {Token, Transfer, Transaction} from "@/app/assets/interfaces";
import moment from "moment";
import axios from "axios";

const zkSyncTxnsFetch = async (address: string): Promise<Transaction[]> => {
    const limit = 100;
    ...
    const fetchTransactionsPage = async () => {
        try {
            const transactionResponse = await axios.get(
                `https://block-explorer-api.mainnet.zksync.io/transactions?address=${address}&limit=${limit}&page=${currentPageForTx}`
            );
    ...
    const fetchTransfersPage = async () => {
        try {
            const transferResponse = await axios.get(
                `https://block-explorer-api.mainnet.zksync.io/address/${address}/transfers?limit=${limit}&page=${currentPageForTransfer}`
            );
            ...
            await Promise.all(
                items.map(async (item: any) => {
                    if (
                        ...
                        !tokenAddresses.some(
                            (token: Token) =>
                                token.contractAddress === item.tokenAddress
                        ) &&
                        item.type !== "mint"
                    ) {
                        try {
        ...
        const transactionsList: Transaction[] = allTransactionData.map(
            (transaction: any) => {
                const transfer = allTransferData.find(
                    (transfer: any) =>
                        transfer.transactionHash === transaction.hash &&
                        transfer.type !== "mint" &&
                        transfer.token !== null &&  // Add null check for transfer.token
                        transfer.token.symbol !== "ETH"
                );
                ...
                const value =
                    transaction.value !== "0"
                        ...
                        : transfer
                            ...
                            : mintTransfer
                                ? `NFT (${mintTransfer.fields.tokenId.length > 6 ? mintTransfer.fields.tokenId.slice(0, 6) + "..." : mintTransfer.fields.tokenId})` : "";
                ...
                const types = allTransferData.map((transfer: any) => {
                    if (transfer.transactionHash === transaction.hash) {
                        if (transfer.type === "mint") {
                            ...
                            transaction.isL1Originated === true
                        ) {
                            return "bridgeIn";
                ...
                const type = types.includes("mint")
                    ? "mint"
                    : types.includes("bridgeIn")
                        ? "bridgeIn"
                        : types.includes("bridgeOut")
                            ? "bridgeOut"
            ...
            if (transaction.value !== "") {
                ...
                if (token) {
                    const value = parseFloat(
                        transaction.value.split(" ")[0].replace(",", "")
                    );
...
export default zkSyncTxnsFetch;
